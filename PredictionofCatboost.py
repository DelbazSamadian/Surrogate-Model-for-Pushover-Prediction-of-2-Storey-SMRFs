...
# # Load the trained CatBoost model
# model_path = r"F:\ML\Pushover-2story\Codes\Omega\RandomForest_Model.pkl"
...
# Load the trained CatBoost model
model_path = r"F:\ML\Pushover-2story\Codes\Omega\CATBoost.cbm"
catboost_best_model = CatBoostRegressor()
catboost_best_model.load_model(model_path)


# Load mean and standard deviation values
mean_std_file_path = "F:\ML\Pushover-2story\Codes\Omega\mean_std_values.csv"
...
def predict_output():
    try:
        ...
        user_input = {
            ...
            'ϴp_Beam1': pd.to_numeric(entry_widgets['ϴp_Beam1'].get(), errors='coerce'),
           'ϴp_Beam2': pd.to_numeric(entry_widgets['ϴp_Beam2'].get(), errors='coerce'),
            'Es': pd.to_numeric(entry_widgets[ 'Es'].get(), errors='coerce'),
            'ϴpc_Col_In1': pd.to_numeric(entry_widgets['ϴpc_Col_In1'].get(), errors='coerce'),
            'ϴp_Col_Ex2': pd.to_numeric(entry_widgets['ϴp_Col_Ex2'].get(), errors='coerce'),
            'ϴp_Col_In1': pd.to_numeric(entry_widgets['ϴp_Col_In1'].get(), errors='coerce')
        }
...
title_frame = tk.Frame(title_frame_outer, bg='blue', bd=2, relief=tk.GROOVE)
title_frame.grid(row=0, column=1, padx=10, pady=5, sticky="n")  # Adjust column and row based on your layout

title_label = tk.Label(title_frame, text="Surrogate Model for Prediction of Omega for 2-Story SMRF", font=('Times New Roman', 20, 'italic'), fg='black', bg='lightsteelblue')  # Set font to Times New Roman, size to 16, and style to italic
title_label.pack()

# Create and place input labels, entry widgets, and description boxes
input_labels = ['Lbay',	'Hstory',	'Mbuilding',	'Fy',	'ϴp_Beam1',	'ϴp_Beam2',	'Es',	'ϴpc_Col_In1',	'ϴp_Col_Ex2',	'ϴp_Col_In1',
] #ϴpc_Col_In1	Hstory	Icol_Ex1	Ibeam1	Lbay	Zcol_Ex1	ϴp_Beam1	Es	Icol_In2	Mbuilding

descriptions = [
    "Length of bay (inch)",
    "Height of first story (inch)",
    "Mass of building (lb)",
    "Yield strength of the material (ksi)",
    "Peak rotation of beams at the first story",
    "Peak rotation of beams at the second story",
    "Modulus of elasticity (ksi)",
    "Pre-peak rotation of interior column at the first story",
    "Peak rotation of exterior column at the second story",
    "Peak rotation of interior column at the first story",
